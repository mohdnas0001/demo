...
describe("SignUpComponent E2E Test", () => {
  ...
  it("should display the sign-up page elements", () => {
    ...
    cy.get("a")
      .contains("Already have an account? Login here.")
      .should("exist");
  });
  ...
  it("should show error for password mismatch", () => {
    ...
    cy.contains("Passwords do not match. Please try again.").should(
      "be.visible",
    );
  });
  ...
  it("should show error for existing user during sign-up", () => {
    ...
    cy.get('button[type="submit"]').click();
    cy.wait("@signupRequest");
    cy.contains("Username already exists.", { timeout: 2000 }).should(
      "be.visible",
    );
  });
  ...
  it("should show error for network issues during sign-up", () => {
    ...
    cy.wait("@signupRequest");
    cy.contains("Network error. Please check your connection.").should(
      "be.visible",
    );
  });
});
